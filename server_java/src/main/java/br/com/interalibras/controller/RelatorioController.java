...
public class RelatorioController {
    ...
    public ResponseEntity<?> deduplicarPontuacoes() {
        ...
        return ResponseEntity.ok(Map.of("removidos", toDelete.size(), "restantes", all.size() - toDelete.size()));
    }

    @GetMapping("/turma/{turmaId}")
    @Transactional(readOnly = true)
    public ResponseEntity<?> relatorioTurma(@PathVariable Long turmaId) {
        Optional<Turma> optTurma = turmaRepository.findById(turmaId);
        if (optTurma.isEmpty()) {
        ...
        for (Usuario aluno : alunos) {
            ...
            aMap.put("avatar", aluno.getAvatar() != null ? aluno.getAvatar() : "assets/avatar/avatar_1.jpg");
    ...
    public ResponseEntity<?> relatorioAluno(@PathVariable Long alunoId) {
        Optional<Usuario> optAluno = usuarioRepository.findById(alunoId);
        if (optAluno.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        ...
        result.put("avatar", aluno.getAvatar() != null ? aluno.getAvatar() : "assets/avatar/avatar_1.jpg");
    ...
    private Map<String, Object> mapPontuacaoItem(Pontuacao p) {
